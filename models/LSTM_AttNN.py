...
# -*- coding: utf-8 -*-
"""
This module contains the source code of the LSTM-based addative (NN) attention model (LSTM_AttNN).
...
class LSTM_AttNN(NMT):
    ...
    def __init__(self, embed_size: int, hidden_size: int, dropout_rate: float, vocab: Vocab, *args, **kwargs):
        ...
        # i.e. a concatenation of the forward and reverse hidden states from the bidirectional LSTM
        # h_{t}^{dec} and c_{t}^{dec} being the current hidden state and cell of the decoder LSTM at the
        # current timestep t, which change each time step (while h_{i}^{enc} does not)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]], eps: float = 0.0) -> torch.Tensor:
        ...
        # what the model thinks should come afterward. Hence log_prob[:, :-1, :] aligns with the true Y_t
    ...
    def encode(self, source_padded: torch.Tensor,
        ...
        """
        Apply the bidirectional LSTM encoder to a collection of padded source sentences of size (b, src_len)
        ...
        # each timestep (i.e. word in each sentence) for both directions, it will also return the last hidden
        # and cell of the encoder each having a size of (2, b, h) for the bidirectional design
        ...
        # Combine the initialized h_{0}^{decoder} and c_{0}^{decoder} as the decoder init state
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        the true Y_t words provided in the target translation as inputs at each time step instead of the
        prior Y_hat_values provided from the prior step. This method is used for training only.

        Parameters
        ----------
        enc_hiddens : torch.Tensor
            A tensor of bidirectional hidden states from the encoder of size (b, src_len, h*2).
    ...
    def step(self, Ybar_t: torch.Tensor, dec_state: Tuple[torch.Tensor, torch.Tensor],
        ...
        enc_hiddens : torch.Tensor
            ...
            src_len = maximum source length, h = hidden size. h * 2 since the encoder is bidirectional.
        ...
        # scores, which should be more expressive than other simpler calculation methods e.g. multiplicative
    ...
    def translate(self, src_sentences: Union[List[str], List[List[str]]], beam_size: int = 1,
        ...
        with torch.no_grad():  # no_grad() signals backend to throw away all gradients
            ...
            # sentence and also the decoder initial hidden state (h of t minus 1) for each sentence
                ...
                if k_pct is None:  # Select the word with the highest modeled probability always
                    ...
                    for i in range(b):
                        if finished_flags[i] == 0:  # Compute if this sentence is not already finished
                            ...
                            bool_vec = sorted_probs.values.cumsum(0) <= k_pct  # The entries in the top k %
                            bool_vec[
                                0] = True  # Always have at least 1 entry set to true i.e. this happens if
                            # the most likely word has a higher prob than k
                            idx, prob = sorted_probs.indices[bool_vec], sorted_probs.values[bool_vec]
                            prob /= prob.sum()  # Re-normalize to 1 and then sample to get the next prediction
                            Y_hat_t[i] = idx[prob.multinomial(num_samples=1, replacement=True).item()]
