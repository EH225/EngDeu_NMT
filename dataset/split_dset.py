...
splits them into separate csv files (train, validation, test) for English (eng) and German (Deutsch -> deu)
...
Certain data quality screening heuristics are used to eliminate bad data. If any applies to either sentence
in the data set, then the sentence pair is dropped. They include:
    - Each sentence can have at most 100 space separated words
    - Each sentence can be at most 800 in total character length
    - NAs and duplicates are dropped
    - If the number of words in each one sentence compared to the other differs by too great of a percent
      then both sentences are dropped
    - If the percentage of words recognized by the sentence's language dictionary is too low, then both
...
and should be saved into dataset\paired_csv\ directory before running this script.
"""
...
def pct_words_recognized(sentence: str, word_set: Set[str]) -> float:
    """
    ...
    """
    # Remove punctuation, replace with a space, then split on white spaces to create a list of words
...
if __name__ == "__main__":
    ...
    for file_name in os.listdir(os.path.join(BASE_PATH, "paired_csv")):
        ...
        if new_file_name == "train.csv":  # Only apply this filter to the training dataset
            ...
            # Drop sentences where the number of recognized words isn't high i.e. if the English doesn't look
